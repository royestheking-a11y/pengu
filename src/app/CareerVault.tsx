...
export default function CareerVault() {
  ...
  const [activeTab, setActiveTab] = useState<'build' | 'templates' | 'reviews' | 'hubAnalyses'>('build');
...
`;
  ...
  const handleGenerateResume = async () => {
    if (!rawResumeText.trim()) {
      toast.error('Please paste your resume details first.');
      return;
    }

    setIsGeneratingResume(true);
    const toastId = toast.loading('Pengu AI is structuring your Premium Resume...');

    try {
      const { data } = await api.post('/resume-builder/generate', { rawText: rawResumeText });
      setGeneratedResume(data);
      toast.success('Resume Generated Successfully!', { id: toastId });
    } catch (error: any) {
      console.error('Resume gen error', error);
      toast.error(error.response?.data?.message || 'Failed to generate resume', { id: toastId });
    } finally {
  ...
  const handleDownloadGeneratedResume = async () => {
    ...
    try {
      // Import the pro fork that supports Tailwind v4 OKLCH CSS natively
      ...
      // Render canvas natively at 2x scale for high-res crispness
      ...
      // Calculate A4 proportions. A4 size is 210mm x 297mm
      const pdf = new jsPDF({
      ...
      // Fit the image perfectly to A4
      pdf.addImage(imgData, 'JPEG', 0, 0, 210, 297);

      const fileName = `${generatedResume.personal?.name?.replace(/\\s+/g, '_') || 'Premium'}_Resume.pdf`;
      pdf.save(fileName);
  ...
  return (
    <DashboardLayout>
      <SEO
        title="Career Vault & ATS Resume Builder"
        description="Turn your academic assignments into a professional portfolio. Auto-generate ATS-friendly resumes and optimize your LinkedIn profile with Pengu."
        url="https://pengu.work.gd/student/career-vault"
        keywords="student portfolio builder, ATS friendly resume generator, academic to career transition, LinkedIn profile optimization"
      />
          ...
          <nav className="-mb-px flex space-x-8">
            <button
              ...
              className={`
                ...
                ${activeTab === 'build' ? 'border-[#3E2723] text-[#3E2723]' : 'border-transparent text-stone-500 hover:text-stone-700 hover:border-stone-300'}
              `}
            >
              <FileUp className="size-4" /> Build Resume <span className="text-[10px] bg-amber-100 text-amber-800 px-1.5 py-0.5 rounded-sm uppercase tracking-wider">AI</span>
            </button>
            <button
        ...
        {activeTab === 'build' ? (
          ...
          >
            ...
            <div className="flex flex-col h-[calc(100vh-250px)]">
              <div className="bg-white rounded-t-xl border border-stone-200 border-b-0 p-4 shrink-0">
                <h2 className="font-bold text-lg text-stone-900 flex items-center gap-2">
                  <FileText className="size-5 text-[#5D4037]" /> Unstructured Input
                </h2>
                <p className="text-sm text-stone-500 mt-1">
                  Paste your messy messy notes, LinkedIn export, or old CV. Our AI will automatically organize it into the premium layout.
                </p>
              </div>
              <textarea
                value={rawResumeText}
                onChange={(e) => setRawResumeText(e.target.value)}
                placeholder="Paste all your details here... 
Example:
John Doe
Software Engineer
john@email.com
Worked at Google from 2020-2023 as a Senior Dev. I built scalable APIs.
Skills: React, Node, Python
Harvard Grad 2019"
                className="flex-1 w-full p-4 border border-stone-200 bg-stone-50 focus:bg-white resize-none outline-none focus:ring-2 focus:ring-[#5D4037] font-mono text-sm shadow-inner overflow-y-auto"
              />
              <div className="bg-white rounded-b-xl border border-stone-200 border-t-0 p-4 shrink-0 flex items-center justify-between">
                <span className="text-xs font-bold text-stone-400">{rawResumeText.length} characters</span>
                <Button
                  onClick={handleGenerateResume}
                  disabled={isGeneratingResume || !rawResumeText.trim()}
                  className="bg-[#3E2723] hover:bg-[#5D4037]"
                >
                  {isGeneratingResume ? (
                    <><Loader2 className="size-4 mr-2 animate-spin" /> Structuring via AI...</>
                  ) : (
                    <><Rocket className="size-4 mr-2" /> Generate Premium CV</>
                  )}
                </Button>
              </div>
            </div>
            ...
            {/* Right: Live Preview */}
            <div className="bg-stone-200/50 rounded-xl border border-stone-200 p-8 flex flex-col items-center justify-start h-[calc(100vh-250px)] overflow-y-auto relative">
              <div className="w-full flex justify-between items-center mb-6 shrink-0 max-w-[800px]">
                <h3 className="font-bold text-stone-700">Live Preview</h3>
                <Button
              ...
              {!generatedResume ? (
                <div className="m-auto text-center max-w-sm">
                  ...
                  <h4 className="font-bold text-stone-600 mb-2">No Generation Yet</h4>
                  <p className="text-sm text-stone-500">
                    Paste your details and click Generate to see the magic happen.
                  </p>
                </div>
              ) : (
                <div className="transform scale-[0.6] sm:scale-75 md:scale-90 lg:scale-100 origin-top shadow-2xl transition-all">
